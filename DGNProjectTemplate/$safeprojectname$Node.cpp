...
MTypeId $safeprojectname$::id( 0x00001 );

// Example attributes
MObject $safeprojectname$::input;
MObject $safeprojectname$::output;
...
**/
MStatus $safeprojectname$::compute(const MPlug& plug, MDataBlock& data) {
	...
	if(plug == output) {
		...
		if(returnStatus != MS::kSuccess)
			MGlobal::displayError("Node $safeprojectname$ cannot get value\n");
		else {
			...
			MDataHandle outputHandle = data.outputValue($safeprojectname$::output);
...
*		MS::kFailure
**/
MStatus $safeprojectname$::initialize() {
